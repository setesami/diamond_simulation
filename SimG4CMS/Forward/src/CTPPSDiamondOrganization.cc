...
uint32_t CTPPSDiamondOrganization::GetUnitIDCTPPS(const G4Step* aStep) const
{
  return const_cast<CTPPSDiamondOrganization *>(this)->GetUnitIDCTPPS(aStep);
}
...
uint32_t CTPPSDiamondOrganization::GetUnitIDCTPPS(const G4Step* aStep)
{
  G4VPhysicalVolume* physVol;
        ...
        unsigned int detector = 0;
                  ...
                  const G4VTouchable* touch = aStep->GetPreStepPoint()->GetTouchable();
                    ...
                    if(physVol->GetName() == "CTPPS_Diamond_Segment")
                     ...
                     //Added by Shirin
                     if (physVol->GetName() == "CTPPS_Diamond_Plane")
                     {
                       plane  =  physVol->GetCopyNo();
         }
                     //Changed by Shirin

                     else if(physVol->GetName() == "CTPPS_Timing_Cylinder_Primary_Vacuum")                                                                                                  {
                     int cpy_no = physVol->GetCopyNo();
                     arm = (cpy_no/666)%10;
                     station = (cpy_no/6)%10;
                     roman_pot =cpy_no%10 ;              }
                     ...
                     return CTPPSDiamondDetId(arm, station, roman_pot, plane, detector).rawId(); //FIXME
}
